...
const interactionEvents = ['pointerdown', 'touchstart', 'mousemove', 'mousedown', 'keydown', 'wheel', 'scroll'];
const interactionPassiveOptions = { passive: true, once: true };
let globalInteractionQueue = [];
let globalInteractionActivated = false;
let globalInteractionListenersSetup = false;

function runGlobalInteractionTasks() {
  if (globalInteractionActivated) return;
  globalInteractionActivated = true;
  
  // Remove all interaction listeners
  if (globalInteractionListenersSetup) {
    interactionEvents.forEach(evt => {
      window.removeEventListener(evt, runGlobalInteractionTasks, interactionPassiveOptions);
    });
    window.removeEventListener('load', runGlobalInteractionTasks);
    globalInteractionListenersSetup = false;
  }
  
  // Execute all queued functions
  const queue = [...globalInteractionQueue];
  ...
  if (queue.length > 0) {
    queue.forEach(fn => {
      try {
        ...
        console.warn('[Global] Interaction task error:', e);
      }
    });
  }
}
...
function setupGlobalInteractionListeners() {
  if (globalInteractionListenersSetup || globalInteractionActivated) return;
  globalInteractionListenersSetup = true;
  
  // Wait for DOMContentLoaded, load, or interaction
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', runGlobalInteractionTasks, { once: true });
  }
  window.addEventListener('load', runGlobalInteractionTasks, { once: true });
  interactionEvents.forEach(evt => {
    window.addEventListener(evt, runGlobalInteractionTasks, interactionPassiveOptions);
  });
  
  // Fallback: if no interaction after 2 seconds, run anyway
  setTimeout(() => {
    if (!globalInteractionActivated && globalInteractionQueue.length > 0) {
      runGlobalInteractionTasks();
    }
  }, 5000);
}
...
function deferUntilInteraction(fn) {
  if (globalInteractionActivated) {
    // Already activated, run immediately
    try {
      fn();
    } catch (e) {
      console.warn('[Global] Deferred function error:', e);
    }
  } else {
    // Add to queue
    globalInteractionQueue.push(fn);
...
class HeaderComponent extends HTMLElement {
  ...
  connectedCallback() {
    this.resizeObserver = new ResizeObserver(([entry]) => {
      ...
      this._resizeRaf = requestAnimationFrame(() => {
        ...
        this._headerTop = this.getBoundingClientRect().top;
      });
    });
  ...
  disconnectedCallback() {
    ...
    document.removeEventListener("scroll", this._windowScrollListener);
  ...
  handleWindowScroll = () => {
    ...
    if (this.stickyMode === "always") {
      const isAtTop = scrollTop <= (this._headerTop || 0);
    ...
    if (isScrollingUp) {
      ...
      if (scrollTop <= (this._headerTop || 0)) {
        this.offscreen = false;
  ...
  init() {
    if (!this._windowScrollListener) {
      this._windowScrollListener = this.handleWindowScroll;
    }
    ...
    if (this.stickyMode) {
      ...
      if (this.stickyMode === "scroll-up" || this.stickyMode === "always") {
        document.addEventListener("scroll", this._windowScrollListener);
      }
    }
  }
}
